...
resource "aws_secretsmanager_secret" "db_credentials" {
  name                    = "counsel-aurora-credentials-${random_id.suffix.hex}"
...
resource "aws_db_subnet_group" "aurora" {
  name       = "counsel-aurora-subnet-group"
...
resource "aws_security_group" "aurora" {
  name        = "counsel-aurora-sg"
  description = "Security group for Counsel Aurora cluster"
...
resource "aws_rds_cluster" "aurora" {
  cluster_identifier     = "counsel-aurora-cluster"
  engine                 = "aurora-postgresql"
  engine_mode            = "provisioned"
  engine_version         = "15.12"
  database_name          = "counsel"
  master_username        = "counseladmin"
...
resource "aws_rds_cluster_instance" "aurora" {
  identifier          = "counsel-aurora-instance-1"
...
resource "aws_iam_role" "lambda_aurora_role" {
  name = "counsel-lambda-aurora-role"
...
resource "aws_iam_role_policy" "lambda_aurora_policy" {
  name = "counsel-lambda-aurora-policy"
