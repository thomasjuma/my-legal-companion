...
resource "aws_s3_bucket" "vectors" {
  bucket = "counsel-vectors-${data.aws_caller_identity.current.account_id}"
...
resource "aws_iam_role" "lambda_role" {
  name = "counsel-ingest-lambda-role"
...
resource "aws_iam_role_policy" "lambda_policy" {
  name = "counsel-ingest-lambda-policy"
...
resource "aws_cloudwatch_log_group" "lambda_logs" {
  name              = "/aws/lambda/counsel-ingest"
  retention_in_days = 7
...
resource "aws_api_gateway_rest_api" "api" {
  name        = "counsel-api"
  description = "Counsel, the legal advisor API"
