...
# ECR repository for the assistant Docker image
...
resource "aws_iam_role" "app_runner_instance_role" {
  name = "counsel-app-runner-instance-role"
...
resource "aws_iam_role_policy" "app_runner_instance_bedrock_access" {
  name = "counsel-app-runner-instance-bedrock-policy"
...
# App Runner service
resource "aws_apprunner_service" "assistant" {
  ...
  source_configuration {
    ...
    image_repository {
      image_identifier      = "${aws_ecr_repository.assistant.repository_url}:latest"
      image_configuration {
        port = "8000"
        runtime_environment_variables = {
          OPENAI_API_KEY    = var.openai_api_key
          COUNSEL_API_ENDPOINT = var.counsel_api_endpoint
          COUNSEL_API_KEY      = var.counsel_api_key
        }
      }
...
resource "aws_iam_role" "eventbridge_role" {
  ...
  name  = "counsel-eventbridge-scheduler-role"
...
# Lambda function for invoking assistant
resource "aws_lambda_function" "scheduler_lambda" {
  count         = var.scheduler_enabled ? 1 : 0
  function_name = "legal-assistant-scheduler"
  ...
  environment {
    variables = {
      APP_RUNNER_URL = aws_apprunner_service.assistant.service_url
    }
  }
...
resource "aws_iam_role" "lambda_scheduler_role" {
  count = var.scheduler_enabled ? 1 : 0
  name  = "counsel-scheduler-lambda-role"
...
resource "aws_scheduler_schedule" "assistant_schedule" {
  count = var.scheduler_enabled ? 1 : 0
  name  = "legal-assistant-schedule"
...
resource "aws_lambda_permission" "allow_eventbridge" {
  ...
  source_arn    = aws_scheduler_schedule.assistant_schedule[0].arn
}
