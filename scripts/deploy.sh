...
build_and_push_backend() {
  ...
  local ecr_url
  ecr_url="$(require_tf_output_raw "$api_tf_dir" "ecr_repository_url" "Apply terraform/api first.")"
...
upload_frontend() {
  ...
  local bucket_name
  bucket_name="$(require_tf_output_raw "$fe_tf_dir" "s3_bucket_name" "Apply terraform/frontend first.")"
  local dist_id
  dist_id="$(cd "$fe_tf_dir" && terraform output -raw cloudfront_distribution_id 2>/dev/null || true)"
  local cloudfront_url
  cloudfront_url="$(cd "$fe_tf_dir" && terraform output -raw cloudfront_url 2>/dev/null || true)"
  local apprunner_url
  apprunner_url="$(cd "$fe_tf_dir" && terraform output -raw apprunner_service_url 2>/dev/null || true)"
