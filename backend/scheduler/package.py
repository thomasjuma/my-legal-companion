...
def create_deployment_package():
    ...
    # Copy Lambda handler (must match Terraform handler: lambda_function.handler)
    print("Copying Lambda function code...")
    lambda_entry = current_dir / 'lambda_function.py'
    if not lambda_entry.is_file():
        print(
            "Error: lambda_function.py not found in this directory. "
            "It is required for the scheduler Lambda entrypoint."
        )
        sys.exit(1)
    shutil.copy2(lambda_entry, package_dir / 'lambda_function.py')
