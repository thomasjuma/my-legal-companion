...
def main():
    ...
    # Check if Docker is running (do not use run_command here — it calls sys.exit, so try/except never runs)
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        shutil.copytree(api_dir, api_package, ignore=shutil.ignore_patterns(
            "__pycache__", "*.pyc", ".env*", "*.zip", "package_docker.py", "test_*.py", "lambda_handler.py"
        ))
        ...
        # Copy database package: must live at /var_task/database/ so `import database` works
        # (a `src/database/` layout is only importable if `src` is on PYTHONPATH, which it is not).
        database_src = backend_dir / "database" / "src" / "database"
        database_dst = package_dir / "database"
        ...
        with open(requirements_file, "w") as f:
            # Keep in sync with backend/api/pyproject.toml and backend/database/pyproject.toml
            f.write("fastapi>=0.115.0\n")
            f.write("uvicorn[standard]>=0.32.0\n")
            f.write("mangum>=0.19.0\n")
            f.write("boto3>=1.26.0\n")
            f.write("fastapi-clerk-auth>=0.0.9\n")
            f.write("pydantic>=2.0.0\n")
            f.write("python-dotenv>=1.0.0\n")
            f.write("sqlalchemy[asyncio]>=2.0.0\n")
            f.write("psycopg[binary]>=3.0.0\n")
            f.write("openai-agents[litellm]>=0.14.0\n")
...
# Default Lambda handler (zip deploy uses the same; see terraform)
